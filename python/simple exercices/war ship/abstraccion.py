...
def filtro2(funcion, unaLista, n):
    if unaLista==listaVacia:
        return listaVacia 
    else:
        if (funcion(cabeza(unaLista),n)):
            return crearLista(cabeza(unaLista),filtro2(funcion, cola(unaLista),n))
        else:
            return filtro2(funcion,cola(unaLista),n)
        
# Tests
valores = lista(6, lista(4, lista(8, listaVacia)))
assert filtro2(lambda x,y: x < y, valores,5) == lista(4, listaVacia) 
